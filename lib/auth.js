    ...
    callbacks: {
        ...
        async jwt({ token, user, account, trigger, session: sessionData }) {
            if (account && user) {
                ...
                // Đồng bộ lại role/department mới nhất từ DB mỗi khi NextAuth làm mới token
                // (tránh JWT giữ role cũ suốt 8h nếu admin đổi quyền cho user sau khi họ đã đăng nhập)
                const dbUser = await prisma.user.findUnique({
                    where: { id: token.id },
                    select: { role: true, department: true, active: true, allowedRoles: true },
                });
                if (dbUser?.active) {
                    token.role = dbUser.role;
                    token.department = dbUser.department || '';
                    token.allowedRoles = (() => { try { return JSON.parse(dbUser.allowedRoles || '[]'); } catch { return []; } })();
                }
            }
            // Xử lý đổi phòng ban
