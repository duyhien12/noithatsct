...
export function getMfgPermissions(user) {
    ...
    if (role === 'xuong') {
        ...
        if (XUONG_QUAN_DOC.includes(department)) {
            ...
            perms.create = true; perms.update = true;
...
/**
 * Quản lý xưởng (Quản đốc/Quản lý/Trưởng phòng/Phó phòng thuộc role 'xuong').
 * Dùng để cấp quyền chấm công/OT cho cấp quản lý xưởng, tách biệt với Ban giám đốc.
 * @param {{ role?: string, department?: string }} user
 */
export function isXuongManager(user) {
    return user?.role === 'xuong' && XUONG_QUAN_DOC.includes(user?.department || '');
}

/**
 * Ai được chấm công/ghi tăng ca xưởng: Ban giám đốc/admin, hoặc quản lý xưởng.
 * @param {{ role?: string, department?: string }} user
 */
export function canManageAttendance(user) {
    return BAN_GD.includes(user?.role) || user?.role === 'admin' || isXuongManager(user);
}
