...
export async function generateCode(model, prefix, padLength = 3) {
    ...
    const records = await prisma.$queryRawUnsafe(
        `SELECT code FROM ${table} WHERE code LIKE ${placeholder}`,
        `${prefix}%`
    );
...
/**
 * Generate next code scoped to the current day, e.g. "PDH-TK-20260708-001".
 * Sequence resets to 001 each day (prefix includes the date, so LIKE-scan is naturally scoped to today).
 *
 * @param {string} model - Prisma model name
 * @param {string} prefix - Code prefix before the date (e.g. 'PDH-TK')
 * @param {number} padLength - Number of digits (default: 3)
 */
export async function generateDailyCode(model, prefix, padLength = 3) {
    if (!TABLE_MAP[model]) throw new Error(`Unknown model "${model}" in generateCode`);
    ...
    const existing = new Set(records.map(r => r.code));
    const maxNum = records
        .map(r => r.code.slice(fullPrefix.length))
        .filter(s => /^\d+$/.test(s))
        .reduce((max, s) => Math.max(max, Number(s)), 0);

    let candidate = maxNum + 1;
    while (existing.has(`${fullPrefix}${String(candidate).padStart(padLength, '0')}`)) {
        candidate++;
    }
    return `${fullPrefix}${String(candidate).padStart(padLength, '0')}`;
}

/**
 * Retry wrapper for generateDailyCode — same semantics as withCodeRetry.
 */
export async function withDailyCodeRetry(model, prefix, createFn, padLength = 3) {
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            const code = await generateDailyCode(model, prefix, padLength);
            return await createFn(code);
        } catch (err) {
            if (err.code === 'P2002' && attempt < MAX_RETRIES - 1) {
                continue;
