        ...
        try {
            ...
            if (!options.public) {
                try { session = await getServerSession(authOptions); }
                    ...
                    console.error('[withAuth] getServerSession lỗi:', authErr.message);
                    return NextResponse.json({ error: 'Auth error: ' + authErr.message }, { status: 500, headers: { 'x-auth-error': authErr.message.slice(0,100) } });
                }
                if (!session) {
            ...
            return NextResponse.json(
                { error: 'Lỗi hệ thống: ' + errMsg },
                { status: 500, headers: { 'x-error-detail': errMsg.slice(0, 200) } }
            );
        }
    };
}
