...
const prepChecklistItemSchema = z.object({
    label: z.string(),
    status: z.enum(['pending', 'passed', 'failed']).default('pending'),
    note: optStr,
}).strict();

const cuttingListItemSchema = z.object({
...
export const mfgOrderCreateSchema = z.object({
    ...
    qcManagerId: optStr,
    note: optStr,
    prepChecklist: z.array(prepChecklistItemSchema).optional(),
    cuttingList: z.array(cuttingListItemSchema).optional(),
}).strict();
