...
import { ADVANCE_CATEGORY_BY_TYPE, SETTLEMENT_CATEGORY_BY_TYPE } from '@/lib/employeeAdvance';
...
const SETTLEMENT_CATEGORY_NAMES = [...new Set(Object.values(SETTLEMENT_CATEGORY_BY_TYPE))];
...
function reverseSettlementType(categoryName) {
    const entry = Object.entries(SETTLEMENT_CATEGORY_BY_TYPE).find(([, catName]) => catName === categoryName);
    return entry ? entry[0] : 'Khác';
}
...
/**
 * Phiếu Thu nhập TRỰC TIẾP trong Nhật ký Thu – Chi (không qua nút "Hoàn ứng" ở chi tiết nhân viên)
 * nhưng đúng danh mục "Thu hoàn ứng" + gắn đối tượng Nhân viên — coi như khoản hoàn ứng "mồ côi"
 * (chưa có AdvanceSettlement liên kết). Không match vào 1 EmployeeAdvance cụ thể (schema bắt buộc
 * advanceId nên không tạo bản ghi AdvanceSettlement thật được) — chỉ cộng thẳng vào tổng "Đã hoàn
 * ứng" cấp nhân viên, giống cách orphan advance cộng vào tổng "Phát sinh".
 *
 * @param {{ employeeId?: string, advanceTypes?: string[] }} filter
 */
export async function findOrphanSettlementTransactions({ employeeId, advanceTypes } = {}) {
    const categoryNames = advanceTypes
        ? advanceTypes.map(t => SETTLEMENT_CATEGORY_BY_TYPE[t]).filter(Boolean)
    ...
    const rows = await prisma.financeTransaction.findMany({
        where: {
            ...
            objectId: { not: '' },
            categoryId: { in: categories.map(c => c.id) },
            settlementOf: null, // chưa liên kết AdvanceSettlement nào (tránh đếm trùng phiếu tạo qua modal)
            ...(employeeId ? { objectId: employeeId } : {}),
        },
        select: {
            id: true, code: true, date: true, amount: true, content: true, categoryId: true,
            objectId: true, objectName: true, attachments: true, status: true,
        },
        orderBy: { date: 'asc' },
    });
    ...
    return rows.map(t => ({
        ...
        date: t.date,
        amount: t.amount,
        settleType: 'cash_return',
        advanceType: reverseSettlementType(categoryNameById[t.categoryId]),
        content: t.content,
        code: t.code,
        financeTransactionId: t.id,
        financeTransactionCode: t.code,
