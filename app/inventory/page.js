...
export default function InventoryPage() {
    ...
    const totalStockValue = stockData.products.reduce((s, p) => s + (p.stock || 0) * (p.salePrice > 0 ? p.salePrice : Math.round((p.importPrice || 0) * 1.08)), 0);
    return (
        <div>
            {/* KPI */}
                                        ...
                                        {stockFiltered.map(p => {
                                            ...
                                            return (
                                                <tr key={p.id} style={{ background: isOut ? 'rgba(239,68,68,0.04)' : isLow ? 'rgba(245,158,11,0.04)' : undefined }}>
                                                    ...
                                                    <td style={{ textAlign: 'right', fontSize: 13 }}>{fmt(p.salePrice > 0 ? p.salePrice : Math.round((p.importPrice || 0) * 1.08))}</td>
                                                    <td style={{ textAlign: 'right', fontWeight: 600 }}>{fmt((p.stock || 0) * (p.salePrice > 0 ? p.salePrice : Math.round((p.importPrice || 0) * 1.08)))}</td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td style={{ textAlign: 'right', fontWeight: 700, padding: '8px 16px' }}>
                                                    {fmt(stockFiltered.reduce((s, p) => s + (p.stock || 0) * (p.salePrice > 0 ? p.salePrice : Math.round((p.importPrice || 0) * 1.08)), 0))}
                                                </td>
                                                <td /><td />
                                            </tr>
                                        </tfoot>
